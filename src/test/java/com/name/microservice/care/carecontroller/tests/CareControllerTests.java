package com.name.microservice.care.carecontroller.tests;
...
import com.name.microservice.care.carecontroller.api.CareController;
import com.name.microservice.care.carecontroller.data.CareControllerData;
import com.name.microservice.care.carecontroller.dataproviders.CareControllerDataProviders;
import com.name.microservice.care.carecontroller.dto.CareContentDTO;
import com.name.microservice.care.carecontroller.dto.CareLocationsDTO;
import com.name.microservice.care.carecontroller.dto.CareOptionDTO;
import com.name.microservice.care.carecontroller.dto.CarePubDetailRequest;
import com.name.microservice.care.carecontroller.dto.CarePubDetailResponse;
import com.name.microservice.care.carecontroller.dto.CareSearchFilterResponse;
import com.name.microservice.care.carecontroller.dto.CareSearchRequest;
import com.name.microservice.care.carecontroller.dto.CareSearchResponse;
import com.name.microservice.care.carecontroller.dto.CareTypeDTO;
...
import static com.name.microservice.care.carecontroller.data.CareControllerData.expCrestorSearch;
import static com.name.microservice.care.carecontroller.data.CareControllerData.noMatchingFromSearch;
...
public class CareControllerTests extends BaseClass {
    ...
    public void certainCareShouldNotShowUpInCareSearch(LoginData loginData, String scCode) throws Exception {
        SoftAssert softAssert = new SoftAssert();
        log.info("When: public search Crestor");
        ...
        CareSearchResponse careSearchResponse = careController.careSearch(
                CareSearchRequest.builder()
                        .memberUuid(loginSession.getSub()).query(scCode).build(),
                loginSession.getToken());

        log.info("Then: Correct Content values should be returned");
        softAssert.assertEquals(careSearchResponse, noMatchingFromSearch(),
                scCode + " is searchable on " + loginData.getUsername());
        softAssert.assertAll();
    }
}
